        ...
        BEGIN
            ...
            LOOP
                ...
                INSERT INTO public.hpp_polls_ops as hppf(
                    ppop_id, block_num, transaction_id, req_auths, req_posting_auths, op_type, op_payload)
                VALUES (
                    temprow.ppop_id, temprow.block_num, temprow.transaction_id,
                    temprow.req_auths, temprow.req_posting_auths, _op_type,
                    _op_payload
                );
                -- Update state tables
                PERFORM hpp_polls_update_state(req_posting_auths[1], req_auths[1], _op_type, _op_payload);
            END LOOP;
        END;
        $function$;

CREATE OR REPLACE FUNCTION public.hpp_polls_update_state( _posting_acc VARCHAR(16), _active_acc VARCHAR(16), _op_type VARCHAR, _op_payload JSON )
    RETURNS void
    LANGUAGE plpgsql
    VOLATILE AS $function$
        DECLARE
