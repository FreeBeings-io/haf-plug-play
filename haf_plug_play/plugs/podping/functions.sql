...
CREATE OR REPLACE FUNCTION podping.update(_podping_id BIGINT, _block_num BIGINT, _created TIMESTAMP, _hash BYTEA,  _required_auths VARCHAR(16)[], _required_posting_auths VARCHAR(16)[], _payload JSON)
    RETURNS void
    LANGUAGE plpgsql
    VOLATILE AS $function$
        ...
        BEGIN
            ...
            IF _version = '0.3' THEN
                ...
                LOOP
                    --RAISE NOTICE '%', _url;
                    INSERT INTO podping.updates(
                        podping_id, block_num, created, trx_id, req_auths,
                        req_posting_auths, url, reason, medium)
                    VALUES (
                        _podping_id, _block_num, _created, _hash, _required_auths,
                        _required_posting_auths, _url, _reason, 'podcast');
                END LOOP;
            ELSIF _version = '1.0' THEN
                ...
                LOOP
                    ...
                    VALUES (
                        _podping_id, _block_num, _created, _hash, _required_auths,
                        _required_posting_auths, _url, _reason, _medium);
                END LOOP;
            END IF;
        END;
    $function$;
...
CREATE OR REPLACE FUNCTION podping.process_cjop(_block_num INTEGER, _created TIMESTAMP, _hash BYTEA, _body JSON)
    ...
    VOLATILE AS $function$
        ...
        BEGIN
            ...
            IF _op_id IN ('podping','pp_video_update', 'pp_podcast_live', 'pp_podcast_update') THEN
                -- save op
                _saved_id := podping.save_op(_block_num, _created, _hash, _required_auths, _required_posting_auths, _op_id, _op_payload);
                PERFORM podping.update(
                    _saved_id, _block_num, _created, _hash,
                    _required_auths, _required_posting_auths, _op_payload);
            END IF;
